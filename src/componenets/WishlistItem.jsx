...
function WishlistItem({ id, imgSrc, imgAlt, name, price, quantity, stock }) {
    ...
    return (
        <div className='flex justify-between nunito h-full max-h-25 bg-white border border-gray overflow-hidden rounded'>
            <Link to={`/product/${id}`}>
                <img className='aspect-5/4 max-w-30 object-cover border-r border-gray' src={imgSrc} alt={imgAlt} loading="lazy" />
            </Link>
            <div className="flex flex-col py-2.5 px-3 sm:p-3 w-full">
                ...
                <div className=" flex justify-between mt-auto">
                    ...
                    <button onClick={() => removeFromWishlist(id)} className='flex cursor-pointer text-[tomato]'><RiDeleteBinLine size={14} /></button>
                </div>
            </div>
        </div>
    )
