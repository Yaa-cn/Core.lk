...
function ProductCard({ id, imgSrc, imgAlt, category, name, price, rating }) {
  ...
  return (
    <div className='productCard flex flex-col bg-white shadow-xs nunito'>
      <div className="overflow-hidden aspect-7/5 sm:aspect-5/4">
        <img className='aspect-7/5 sm:aspect-5/4 object-cover' src={imgSrc} alt={imgAlt} loading="lazy" />
      </div>
      <div className='flex flex-col gap-2 sm:gap-1.5 lg:gap-2.5 m-2.5 sm:m-3.5 grow justify-between'>
        ...
        <h5 className={`text-xs sm:text-sm ${location.pathname.includes('/shop') ? 'xl:text-sm' : 'xl:text-base'} font-bold line-clamp-2 sm:min-h-10`}>{name}</h5>

        <div className="flex justify-between">
          <p className={`text-xs ${location.pathname.includes('/shop') ? 'xl:text-xs' : 'xl:text-sm'} font-medium my-auto`}>{price} LKR</p>
          <button onClick={(e) => {addToCart(product)} } className='rounded-full bg-accent w-7 sm:w-8 h-7 sm:h-8 border border-gray cursor-pointer hover:bg-secondary hover:text-white transition ease-in duration-200'><RiShoppingCart2Fill className="m-auto w-3.5 h-3.5 sm:w-4 sm:h-4" /></button>
        </div>
      </div>
    </div>
  )
