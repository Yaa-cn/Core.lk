...
function Addresses() {
    ...
    const { addresses, setAddresses, loading, addAddress, editAddress, deleteAddress } = useProfile()

    const sortedAddress = addresses.sort((a, b) => b.isDefault - a.isDefault)
    ...
    const handleEditAddress = (e) => {
        ...
        setAddresses(prev => {
            prev.map(address =>
                address._id === addressDetails._id ?
                    {
                        ...address,
                        label: label.current.defaultValue,
                        name: name.current.defaultValue,
                        phoneNo: phone.current.defaultValue,
                        address: address.current.defaultValue,
                        city: city.current.defaultValue,
                        district: district.current.defaultValue,
                        isDefault: isDefault.current.defaultChecked
                    } : address
            )
        })
        ...
        <div>
                            ...
                            {sortedAddress.map((address, index) =>
                                <div key={index} className="flex flex-col px-5 py-3 outfit justify-between h-fit bg-accent/40 border border-secondary/20 rounded-[3px] w-full md:max-w-120">
                                    <div className="flex justify-between items-center">
                                        ...
                                        {address.isDefault && <span className="text-[10px] nunito font-bold text-primary/80 border border-secondary/20 bg-secondary/5 px-3 py-0.5 rounded-full select-none ">Default</span>}
                                    </div>
            ...
            <Modal isOpen={visibleAddAddress} onClose={() => setVisibleAddAddress(false)}>
                ...
                <form onSubmit={handleAddAddress} className="flex flex-col gap-4">
                    ...
                    <div className="flex flex-col gap-0.5">
                        ...
                        <input required pattern='[A-Za-z\s]+' ref={name} type="text" name="name" id="name" placeholder="Name" className="text-xs text-primary/80 border border-secondary/20 rounded-[3px] px-4 py-3 outline-none " />
                    </div>

                    <div className="flex flex-col gap-0.5">
                        <label htmlFor="phone" className="text-[10px] uppercase text-neutral-500">Phone Number</label>
                        <input pattern="^\+?\d{7,15}$" required ref={phone} type="text" name="phone" id="phone" placeholder="Number" className="text-xs text-primary/80 border border-secondary/20 rounded-[3px] px-4 py-3 outline-none " />
                    </div>
                    ...
                    <div className="flex flex-col gap-0.5">
                        ...
                        <input required pattern='[A-Za-z\s]+' ref={city} type="text" name="city" id="city" placeholder="City" className="text-xs text-primary/80 border border-secondary/20 rounded-[3px] px-4 py-3 outline-none " />
                    </div>

                    <div className="flex flex-col gap-0.5">
                        <label htmlFor="district" className="text-[10px] uppercase text-neutral-500">District</label>
                        <input required pattern='[A-Za-z\s]+' ref={district} type="text" name="district" id="district" placeholder="District" className="text-xs text-primary/80 border border-secondary/20 rounded-[3px] px-4 py-3 outline-none " />
                    </div>
            ...
            <Modal isOpen={visibleEditAddress} onClose={() => setVisibleEditAddress(false)}>
                ...
                <form onSubmit={handleEditAddress} className="flex flex-col gap-4">
                    ...
                    <div className="flex flex-col gap-0.5">
                        <label htmlFor="name" className="text-[10px] uppercase text-neutral-500">Name</label>
                        <input required pattern='[A-Za-z\s]+' ref={name} type="text" name="name" id="name" defaultValue={addressDetails.name} placeholder="Name" className="text-xs text-primary/80 border border-secondary/20 rounded-[3px] px-4 py-3 outline-none " />
                    </div>

                    <div className="flex flex-col gap-0.5">
                        <label htmlFor="phone" className="text-[10px] uppercase text-neutral-500">Phone Number</label>
                        <input pattern="^\+?\d{7,15}$" required ref={phone} type="text" name="phone" id="phone" defaultValue={addressDetails.phoneNo} placeholder="Number" className="text-xs text-primary/80 border border-secondary/20 rounded-[3px] px-4 py-3 outline-none " />
                    </div>
                    ...
                    <div className="flex flex-col gap-0.5">
                        <label htmlFor="city" className="text-[10px] uppercase text-neutral-500">City</label>
                        <input required pattern='[A-Za-z\s]+' ref={city} type="text" name="city" id="city" defaultValue={addressDetails.city} placeholder="City" className="text-xs text-primary/80 border border-secondary/20 rounded-[3px] px-4 py-3 outline-none " />
                    </div>

                    <div className="flex flex-col gap-0.5">
                        <label htmlFor="district" className="text-[10px] uppercase text-neutral-500">District</label>
                        <input required pattern='[A-Za-z\s]+' ref={district} type="text" name="district" id="district" defaultValue={addressDetails.district} placeholder="District" className="text-xs text-primary/80 border border-secondary/20 rounded-[3px] px-4 py-3 outline-none " />
                    </div>
