...
function NotFound() {
  ...
  return (
    <div className='flex flex-col justify-center items-center gap-3 mx-4 sm:mx-10 h-110'>
      ...
      <button onClick={() => { navigate('/'); scrollTo({ top: 0, behavior: 'smooth' }) }} className='text-[10px] sm:text-xs bg-accent border border-secondary/50 text-secondary px-4 py-2.5 mt-2 rounded-[3px] font-medium outfit uppercase hover:bg-secondary hover:text-light transition-colors duration-300 cursor-pointer'>Go back home</button>
    </div>
  )
}
